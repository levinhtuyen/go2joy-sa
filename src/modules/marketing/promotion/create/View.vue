  ...
  <sa-section v-loading="ui.isCallApi">
    ...
    >
      ...
      >
        ...
        <el-col :span="24" class="pd-l-0">
          ...
          <el-checkbox
            :true-label="1"
            :false-label="0"
            v-model="promotion.callMaxDiscount"
      ...
      <el-form-item
        ...
        prop="content"
      >
        <tinymce v-model="promotion.content" :height="300" />
      </el-form-item>
      <el-form-item
        :label="$t('page.create-promotion.contentEn')"
        prop="contentEn"
      >
        <tinymce v-model="promotion.contentEn" :height="300" />
      </el-form-item>
      <el-form-item
      ...
      <el-form-item
        v-if="promotion && promotion.displayType == 1"
      ...
      <el-form-item
        ...
        :prop="file.label"
      >
        <el-input :value="promotion[file.label]" :name="file.label" class="disappear"/>
        <el-col :span="6" class="pd-l-0">
          ...
          <div>
            <span>
              <span v-if="promotion.displayType == 0"
...
export default {
  ...
  methods: {
      ...
      this.$refs.ruleForm.validate(async (valid) => {
        if (valid) {
          try {
            ...
            formData.append('numOfCoupon', this.promotion.numOfCoupon)
            formData.append('maxNumCoupon', this.promotion.maxNumCoupon)
            formData.append('type', this.promotion.type)
            formData.append('coPromotion', this.promotion.coPromotion)
            formData.append('display', this.promotion.display)
            formData.append('calMaxDiscount', this.promotion.callMaxDiscount)
            formData.append('afterDiscount', this.promotion.applyToFee)
            formData.append('makeLabel', this.promotion.makeLabel)
            formData.append('directDiscount', this.promotion.directDiscount)
            formData.append('displayHotel', this.promotion.displayHotel)
            formData.append('applyStart', this.promotion.applyStart)
            formData.append('applyEnd', this.promotion.applyEnd)
            formData.append('couponStart', this.promotion.couponStart)
            formData.append('couponEnd', this.promotion.couponEnd)
            formData.append('discountType', this.promotion.discountType)
            formData.append('discount', this.promotion.discount)
            formData.append('maxDiscount', this.promotion.maxDiscount)
            formData.append('go2joyDiscount', this.promotion.go2joyDiscount)
            formData.append('hotelDiscount', this.promotion.hotelDiscount)
            formData.append('content', this.promotion.content)
            formData.append('contentEn', this.promotion.contentEn)
            if (this.promotion.type === 2 || this.promotion.type === 3) {
              formData.append('content2', this.promotion.content2)
              formData.append('memo', this.promotion.memo)
            }
            formData.append('typesOfDisplay', this.promotion.displayType)
            if (this.promotion.displayType === 1) {
              formData.append('subContentVi', this.promotion.subContentVi)
              formData.append('subContentEn', this.promotion.subContentEn)
              formData.append('nameOfButton', this.promotion.nameOfButton)
            }
            ...
            formData.append('numActiveDay', this.applyStartEnd === 1 ? this.promotion.numActiveDay : 0)
            formData.append('numOfDays', this.applyStartEnd === 1 ? 1 : 0)
            formData.append('fromToDay', this.applyStartEnd === 0 ? 1 : 0)
    ...
    filterTypeDisplaySelect (data) {
      this.promotion.displayType = data.item.value
    },
    ...
    handleFileUpload (file, e, label) {
      ...
      this.promotion[label] = e.target.files[0].name
