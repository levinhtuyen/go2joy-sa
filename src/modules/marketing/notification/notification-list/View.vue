<template>
  <sa-section v-if="$route.fullPath === '/hotel/sadmin/notification'" class="notification">
    ...
    >
      ...
      >
        <template slot-scope="scope">
            <!-- <router-link
            :to="{ name: scope.row.targetType === 2 ? 'hotelDisplayDetails' : 'hotelDisplayDetails', params: { sn: scope.row.targetSn }}">
              {{ getTarget(value.targetType) }}
            </router-link> -->

          <div v-if="scope.row.type === 3"></div>
          <div v-else>
            <p v-if="scope.row.targetType === 1">{{ $t('option.targetType.notice') }}</p>
            <router-link
              v-else-if="scope.row.targetType === 2"
              :to="{ name: 'notificationDetails', params: { sn: scope.row.sn }}">
              {{ $t('option.targetType.promotion') }}
            </router-link>
            <p v-else-if="scope.row.targetType === 3">{{ $t('option.targetType.serviceAgreement') }}</p>
            <router-link
              v-else-if="scope.row.targetType === 4"
              :to="{ name: 'hotelDisplayDetails', params: { sn: scope.row.targetSn }}">
              {{ $t('option.targetType.hotel') }}
            </router-link>
            <p v-else-if="scope.row.targetType === 5">{{ $t('option.targetType.faq') }}</p>
            <p v-else-if="scope.row.targetType === 6">{{ $t('option.targetType.inviteFriend') }}</p>
            <p v-else-if="scope.row.targetType === 7">{{ $t('option.targetType.openApp') }}</p>
            <p v-else-if="scope.row.targetType === 8">{{ $t('option.targetType.mileagePoint') }}</p>
            <p v-else-if="scope.row.targetType === 9">{{ $t('option.targetType.directDiscount') }}</p>
            <p v-else-if="scope.row.targetType === 10">{{ $t('option.targetType.signUp') }}</p>
            <p v-else-if="scope.row.targetType === 11">{{ $t('option.targetType.promotionGroup') }}</p>
...
import { statusOptionsV3, statusOptionsV4, sendToOptions } from '@/utils/const'
