    ...
    <sa-section>
      <div class="review-mgt__filter">
        <p class="statistics" v-if="data && data.length > 0">
          <a class="el-link el-link--default is-underline" href="review/detail">{{$t('page.reviewMgt.label_header')}}</a>
...
export default {
  ...
  methods: {
    ...
    async listReview (filter) {
      this.ui.isReviewLoading = true
      try {
        if (this.ui.hotel?.status === -1) {
          filter.hotelGroupSn = this.ui.hotel?.sn
        } else {
          filter.hotelSn = this.ui.hotel?.sn
        }
        const params = {
          ...filter
        }
