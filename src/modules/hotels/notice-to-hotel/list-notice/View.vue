...
<div class="dashboard-block">
    <div class="notice-to-hotel-segment" v-loading="ui.loadingNoticeToHotelList">

        <!-- Start Contentbar-->
        <div class="notice-to-hotel-segment__filter">
            <div class="notice-to-hotel-segment__filter--input">
                <el-input :placeholder="$t(`page.noticeList.placeholder`)" v-model="query.keyword" clearable prefix-icon="el-icon-search" @input="handleSearch" />
            </div>
            <div class="notice-to-hotel-segment__filter--select">
              <SaSelectCheck
                  class="select-check"
            ...
            <div class="notice-to-hotel-segment__filter--btn">
                <el-button type="warning" icon="el-icon-plus" @click="onDirectAddNoticeToHotel" plain>{{ $t('button.add') }}</el-button>
            </div>
        </div>
        <el-table
        :data="tableData"
        ...
        :row-class-name="rowContentStyle">
            ...
            <el-table-column :label="$t('page.noticeList.topic')" min-width="8">
            ...
            <el-table-column :label="$t('page.noticeList.message_type')" min-width="10">
                <template slot-scope="scope">
                    <span> {{ getLabelTypeNotice(scope.row.type) }}</span>
                </template>
            </el-table-column>
            <el-table-column :label="$t('page.noticeList.apply_for_hotel')" min-width="10">
                <template slot-scope="scope">
                    <span v-html="getLabelApplyNotice(scope.row.hotelScope, scope.row.numNotAnswer, scope.row.sn, scope.row.faqHotelCount)"></span>
                </template>
            </el-table-column>
            <el-table-column :label="$t('page.noticeList.create_staff')" min-width="10">
                <template slot-scope="scope">
                    <span> {{ scope.row.staffName }}</span>
                </template>
            </el-table-column>
            <el-table-column :label="$t('page.noticeList.create_time')" min-width="10">
                <template slot-scope="scope">
                    <span> {{ formatDate(scope.row.createTime) }}</span>
                </template>
            </el-table-column>
            <el-table-column :label="$t('page.noticeList.last_update')" min-width="10">
                <template slot-scope="scope">
                    <span> {{ formatDate(scope.row.lastUpdate) }}</span>
                </template>
            </el-table-column>
            <el-table-column :label="$t('page.noticeList.operations')" align="center" min-width="5">
                <template slot-scope="scope">
                    <el-popover placement="left" trigger="click">
                        <i slot="reference" class="operations el-icon-more" />
                        <div style="padding: 8px 12px;">
                          <el-button v-if="scope.row.answer" type="primary" plain size="small" @click="onReplyNoticeToHotelById(scope.row.sn)">
                                {{ $t('button.reply')}}
                            </el-button>
                            <el-button type="success" plain size="small" icon="el-icon-s-promotion
" @click="onSendNoticeToHotelById(scope.row.sn)">
                                {{ $t('button.send')}}
                            </el-button>
                            <el-button type="success" plain size="small" icon="el-icon-edit" @click="onEditNoticeToHotelById(scope.row.sn)">
                                {{ $t('button.edit')}}
                            </el-button>
                            <el-button type="danger" plain size="small" icon="el-icon-delete" @click="onDeleteNoticeToHotelById(scope.row.sn)">
                                {{ $t('button.delete')}}
                            </el-button>
                        </div>
                    </el-popover>
        ...
        <div class="notice-to-hotel-segment__footer">
            <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :page-sizes="[10, 20, 30, 40, 50]" :page-size="pagination.perPage" layout="total, sizes, prev, pager, next, jumper" :total="pagination.total">
            </el-pagination>
        </div>
    </div>
</div>
