    ...
    <div class="apply-hotel-notice-segment" v-loading="ui.loadingNoticeToHotelList">
        ...
        <div class="apply-hotel-notice-segment__filter">
            ...
            <div class="apply-hotel-notice-segment__filter--btn">
                <el-button :disabled="filter.applyTarget !== 4 ? true : false" @click="addDataApplyingHotelFaq" type="primary" icon="el-icon-plus"  plain>{{ $t('button.add') }}</el-button>
            </div>
            <div class="apply-hotel-notice-segment__filter--btn-group">
              <el-button @click="listDataApplyingHotelFaq" class="btn btn-primary-rgba" icon="el-icon-menu"  plain>{{ $t('button.list') }}</el-button>
              <el-button @click="saveDataApplyingHotelFaq" class="btn btn-primary-rgba" icon="el-icon-plus"  plain>{{ $t('button.save') }}</el-button>
            </div>
        </div>
        <el-table
        :data="tableData"
        class="apply-hotel-notice-segment__table">
            ...
            <el-table-column :label="$t('page.noticeApplyHotel.hotel_name')" min-width="8">
            ...
            <el-table-column :label="$t('page.noticeApplyHotel.code')" min-width="10">
                <template slot-scope="scope">
                    <span> {{ scope.row.code }}</span>
                </template>
            </el-table-column>
            <el-table-column :label="$t('page.noticeApplyHotel.status')" min-width="10">
                <template slot-scope="scope">
                    <span> {{ getLabelStatusHotel(scope.row.hotelStatus) }}</span>
                </template>
            </el-table-column>
            <el-table-column :label="$t('page.noticeApplyHotel.phone')" min-width="10">
                <template slot-scope="scope">
                    <span> {{ scope.row.phone }}</span>
                </template>
            </el-table-column>
            <el-table-column :label="$t('page.noticeApplyHotel.address')" min-width="10">
                <template slot-scope="scope">
                    <span> {{ scope.row.address }}</span>
                </template>
            </el-table-column>
            <el-table-column :label="$t('page.noticeApplyHotel.create_time')" min-width="10">
                <template slot-scope="scope">
                    <span> {{ formatDate(scope.row.createTime) }}</span>
                </template>
            </el-table-column>
            <el-table-column :label="$t('page.noticeApplyHotel.last_update')" min-width="10">
                <template slot-scope="scope">
                    <span> {{ formatDate(scope.row.lastUpdate) }}</span>
                </template>
            </el-table-column>

            <el-table-column :label="$t('page.noticeApplyHotel.operations')" align="center" min-width="5">
                <template slot-scope="scope">
                    <el-popover placement="left" trigger="click">
                        <i slot="reference" class="operations el-icon-more" />
                        <div style="padding: 8px 12px;">
                            <el-button type="danger" plain size="small" icon="el-icon-delete" @click="onDeleteHotelApply(scope.row.sn)">
                                {{ $t('button.delete')}}
                            </el-button>
                        </div>
                    </el-popover>
...
export default {
  ...
  methods: {
    ...
    async getDataDistricts () {
      this.ui.loadingNoticeToHotelList = true
      try {
        const { data } = await fetchDistricts({ limit: 70, status: 1 })
    ...
    handleSizeChange (value) {
      this.queryAddHotelApply.limit = value
      this.onLoadListHotel(this.queryAddHotelApply)
    },
    // pagination handlePage
    handleCurrentChange (value) {
      this.queryAddHotelApply.page = value
      this.onLoadListHotel(this.queryAddHotelApply)
    },
    // Action delete Hotel
