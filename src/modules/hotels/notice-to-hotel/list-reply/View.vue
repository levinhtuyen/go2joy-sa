...
<div class="dashboard-block">
    <div class="notice-to-hotel-list-segment" v-loading="ui.loadingReplyNoticeList">

        <!-- Start Contentbar-->
        <div class="notice-to-hotel-list-segment__filter">
            <div class="notice-to-hotel-list-segment__filter--btn">
                <el-button type="success" icon="el-icon-menu" @click="onDirectListNotice" plain>{{ $t('button.list') }}</el-button>
            </div>
        </div>
        <el-table
        :data="tableData"
        class="notice-to-hotel-list-segment__table">
            ...
            <el-table-column :label="$t('page.noticeHotelList.hotel_name')" min-width="8">
            ...
            <el-table-column :label="$t('page.noticeHotelList.status')" min-width="10">
                <template slot-scope="scope">
                    <span> {{ setHotelStatus(scope.row.hotelStatus) }}</span>
                </template>
            </el-table-column>
            <el-table-column :label="$t('page.noticeHotelList.phone')" min-width="10">
                <template slot-scope="scope">
                    <span >{{ scope.row.hotelPhone }}</span>
                </template>
            </el-table-column>
            <el-table-column :label="$t('page.noticeHotelList.address')" min-width="10">
                <template slot-scope="scope">
                    <span> {{ scope.row.hotelAddress }}</span>
                </template>
            </el-table-column>
            <el-table-column :label="$t('page.noticeHotelList.create_time')" min-width="10">
                <template slot-scope="scope">
                    <span> {{ formatDate(scope.row.hotelCreateTime) }}</span>
                </template>
            </el-table-column>
            <el-table-column :label="$t('page.noticeHotelList.last_update')" min-width="10">
                <template slot-scope="scope">
                ...
                <template slot-scope="scope">
                    <el-popover placement="left" trigger="click">
                        ...
                        <div style="padding: 8px 12px;">
                          <el-button type="primary" plain size="small" @click="onReplyNoticeToHotelById(scope.row.faqHotelSn,scope.row.hotelSn)">
                                {{ $t('button.reply')}}
                            </el-button>
                        </div>
                    </el-popover>
