    ...
    <sa-section>
      <div class="mar-tb-12">
        ...
        <el-button
          type="warning"
          plain
          @click="$router.push({path: `create-room-type${sn != ' ' ? '?sn='+sn : ''}`})"
        >
          <img
...
<script>
import { listRoomType, getRoomType, onCancelFlashSale, onBlockRoomType, onDeleteRoomType } from '@/api/roomType'
...
export default {
  ...
  methods: {
    ...
    deleteRoomType (id) {
      this.$confirm(
        `${this.$t('table.deleteButton.message_alert')}`,
        `${this.$t('table.deleteButton.message_warning')}`,
        {
    ...
    async handleSwitchClick (arg) {
      if (arg.applyFlashSale === 1) {
        this.$confirm(
          `${this.$t(
            'page.calendar.contentFlashSale'
          )}`,
          `${this.$t(
            'page.calendar.headerFlashSale'
          )}`,
          {
            confirmButtonText: 'OK',
            cancelButtonText: 'Cancel',
    ...
    async onBlockRoomType (arg) {
      try {
        ...
        if (data) {
          ...
          self.$message({
            type: 'success',
            message: `${self.$t('message.success')}`
          })
        }
      } catch (error) {
        return false
